use bitcoin::{
    util::bip158::{BlockFilterWriter, Error},
...
pub fn add_input_scripts<F>(
    writer: &mut dyn FilterWriter,
    txs: &[Transaction],
    script_for_coin: F,
) -> Result<(), Error>
where
    F: Fn(&OutPoint) -> Result<Script, Error>,
{
    ...
    for script in txs
        ...
        .skip(n)
        .flat_map(|t| {
            t.input.iter().filter_map(|i| {
                if i.script_sig.is_empty() {
